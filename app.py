...
def fetch():
    ...
    if(request.method == "POST"):
        ...
        model = pickle.load(open("titanic_svc_model.pkl", "rb"))
        sc = pickle.load(open("age_scaler.pkl","rb"))
        sc1 = pickle.load(open("fare_scaler.pkl","rb"))

        age_ = age
        age_ = np.array(age_) 
        age_ = np.reshape(age_, (1,1))
        age_ = sc.fit_transform(age_)
        age_ = age_[0][0]

        fare_ = fare
        fare_ = np.array(fare_) 
        fare_ = np.reshape(fare_, (1,1))
        fare_ = sc1.fit_transform(fare_)
        fare_ = fare_[0][0]
